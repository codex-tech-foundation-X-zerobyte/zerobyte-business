...
The Admin Console Audit log is loaded through `list-platform-audit`. It combines platform-admin audit records, organization/project `audit_logs`, GitHub pushes and pull requests, GitHub Actions workflow runs (including Pages builds), and deployments. Configure the Edge Function secrets, not Vite variables:

```bash
supabase secrets set GITHUB_REPOSITORY=owner/repository
supabase secrets set GITHUB_TOKEN=github_pat_...
```

`GITHUB_TOKEN` is optional for public repositories, but a fine-grained read-only token is recommended for reliable rate limits and private repositories. The token is only read by the server-side Edge Function and is never returned to the browser. Monitoring reports GitHub as **Healthy**, **Configuration needed**, or **Failed** from this same server-side API response; it does not claim success when the repository or function is unavailable. The GitHub Actions API provides the latest workflow/build status, so Pages build success and failure are visible in Audit log after the function is deployed.

Every Edge Function shares CORS handling from `supabase/functions/_shared/cors.ts`. By default it allows any origin, which keeps local development and first deploys working; before relying on this in production, set the `ALLOWED_ORIGINS` function secret to a comma-separated list of the app's real origin(s):
```bash
...
Admin Console → System Monitoring grades each check on both success *and* latency (Healthy ≤500ms, Degraded ≤1s, Slow ≤2s, Critical beyond that; a definite 4xx/5xx is graded on the error, not the clock). Results persist to `platform_monitoring_measurements` via `record_platform_monitoring_measurements`, which is rate limited to one batch per admin every 5 seconds.

The table has no automatic retention -- it will grow unbounded. `cleanup_platform_monitoring_measurements(retain_days)` deletes anything older than the window (default 30 days) but is not scheduled by this migration. If your project has the `pg_cron` extension enabled, schedule it yourself, e.g.:
```sql
select cron.schedule('cleanup-platform-monitoring', '0 3 * * *', $$select public.cleanup_platform_monitoring_measurements(30)$$);
```
Otherwise, call it manually or from an external scheduler.

Run the unit tests for the health-grading logic (`src/lib/monitoring.ts`) with:
```bash
npm test
```
